  ...
  methods: {
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate(async(valid) => {
        ...
        this.loading = true
        try {
          await verifyCode(this.loginForm.code)
          ...
          await this.$store.dispatch('loginUser', { id: userInfo.id })
          await this.$router.push(this.redirect || '/index')
        } catch (error) {
          // 请求拦截器已统一显示错误信息，失败后刷新验证码供用户重试
          this.loginForm.code = ''
          this.getVerify()
        } finally {
          // 无论验证码、登录或跳转是否成功，都必须恢复登录按钮
          this.loading = false
        }
      })
    }
